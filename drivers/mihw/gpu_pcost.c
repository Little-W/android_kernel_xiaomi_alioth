...
int gpu_ea_start(struct devfreq *devf)
{
	...
	devf->ea_private = kcalloc(1,
			sizeof(struct gpu_devinfo), GFP_NOWAIT);
	...
	ret = devfreq_register_notifier(devf,
			&gdev->nb,
...
int gpu_ea_stop(struct devfreq *devf)
{
	...
	init_gpu_suc = 0;
	devfreq_unregister_notifier(devf,
		&gdev->nb,
		DEVFREQ_TRANSITION_NOTIFIER);
...
{
	int lev;
	struct gpu_devinfo *g_dev = container_of(nb,
					struct gpu_devinfo,
					nb);
...
static ssize_t gpu_ea_reset(struct device *dev,
...
static ssize_t gpu_ea_show(struct device *dev,
                struct device_attribute *attr,
                char *buf)
{
	int i, nr_eas, len = 0;
...
static DEVICE_ATTR(gpu_ea, 0444, gpu_ea_show, gpu_ea_reset);

static const struct device_attribute *gpu_ea_attr_list[] = {
		&dev_attr_gpu_ea,
		NULL
};
