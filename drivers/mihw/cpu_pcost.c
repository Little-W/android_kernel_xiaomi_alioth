...
#define COLLET_WORK(work) \
	container_of(work, struct collet_work, wk);
...
static int freq_index(struct cluster_devinfo *cluster,
		unsigned int freq)
{
	int index;
		...
		unsigned int new_freq)
{
	...
	for_each_cpu(cpu, &clus->cpus) {
		if (init_cpu_suc[cpu]) {
			memset(per_cpu(cpu_st, cpu).atime,
				0, nr_cap * sizeof(u64));
...
static ssize_t read_clus_power(int src_cpu, char *buf)
{
	...
	if (!init_clus_suc[cid])
		return sprintf(buf, "clus init failed %d\n", cid);
	...
	if (unlikely(!policy))
		return sprintf(buf, "get policy failed %d\n", cid);
	...
	for (i = 0; i < nr_cap; i++) {
		fcost = div64_u64(
			atime[i] * clus->cstate[i].power, w_time);
                pcost[i] = fcost;
                pcost[nr_cap] += fcost;

		if (debug_pcost)
			pr_info("%2d p: %15llu a: %15llu w: %15llu ap %15llu\n",
				i, clus->cstate[i].power,
				atime[i], wtime[i], pcost[i]);

		len += snprintf(buf + len, PAGE_SIZE - len,
			"p %15llu a %15llu w %15llu ap %15llu rp %15llu %d\n",
			clus->cstate[i].power,
			atime[i], wtime[i], pcost[i],
			div64_u64(atime[i] * clus->cstate[i].power,
				power_unit),
			   ...
			   size_t count)
{
	...
	if (auto_collet) {
		...
		clus->cwk.len = 0;
		queue_delayed_work(ea_wq, &clus->cwk.wk,
				collet_period_jif);
	}
...
static const struct attribute_group eas_attr_group = {
	.attrs = default_attrs,
	.name = "cpu_pcost"
};
...
static int alloc_percpu_time(int cpu, int nr_cap)
{
	...
	per_cpu(cpu_st, cpu).atime = kcalloc(nr_cap,
		sizeof(u64), GFP_NOWAIT);
...
static int enery_probe(int cluster)
{
	...
	cluster_info[cluster].pcost =
		(u64*)(wtime + nr_cap + 1);

	cluster_info[cluster].cwk.buf =
	...
	for (index = 0; index < nr_cap; index++) {
		...
		pr_info("%2d, P: %8lu, F:%8lu\n", index,
			cstate[index].power, cstate[index].freq);
	}
...
static int cpu_pcost_init(void)
{
	...
	for_each_cpu(i, &cpus) {
		...
		cpumask_copy(&cluster_info[cluster].cpus,
				cluster_cpus);
